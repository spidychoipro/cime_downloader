...
ci.me(씨미) VOD 영상을 내려받는 도구입니다. `v0.4`에서는 4K 비디오 다운로더 스타일의 모던한 UI로 전면 개편하고, `v0.2`에서 넣은 보안 가드를 유지했습니다.
...
- **4K Video Downloader 스타일 UI**: "+ Paste Link" 중심의 직관적인 워크플로우로 개편
- **Smart Mode**: 설정 항목(폴더, 파일명 등)을 필요할 때만 열어볼 수 있도록 숨김 처리
- **그린 테마**: 다크 모드에 최적화된 그린 포인트 컬러(#21a366) 적용
- **로그 토글**: 로그 패널을 접고 펼 수 있게 하여 메인 큐 화면을 더 넓게 활용
- **UI 간소화**: 불필요한 입력창과 버튼을 제거하고 클립보드 연동 강화
